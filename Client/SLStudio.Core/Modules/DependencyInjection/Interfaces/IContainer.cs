...
public interface IContainer : IAsyncDisposable, IDisposable, IServiceProvider
{
    ...
    void AddRegistration<TService>(Registration registration)
    ...
    InstanceProducer? GetRegistration<TService>()
        where TService : class;
    InstanceProducer? GetRegistration<TService>(bool throwOnFailure);
    ...
    IEnumerable<Type> GetTypesToRegister<TService>(IEnumerable<Assembly> assemblies, TypesToRegisterOptions options);
